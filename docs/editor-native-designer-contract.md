...
R3.1 is done: "Zet hier een passende afbeelding." now works inside the Editor. The
embedded Agent submits the instruction plus a bounded, validated editor context
through the existing durable Designer run endpoint; the run resolves one existing
project media asset and returns a typed `insert_image` operation inside the normal
proposal envelope; the Agent surface shows the candidate inline and the Editor
applies it as one undoable change through the existing `insertMedia` command. It
adds no route, no second editor state and no backend document write (the apply
endpoint refuses insertion proposals). The full contract, bounds, error table and
limitations are in `docs/context-aware-image-insertion.md`; the key limitations
are one candidate per request (no "choose another"), no replacement of an existing
image block, and media-library-only sourcing (no generation/search).

Next deliverable: **Phase R4 - In-Editor Proposal, Apply & Undo**, generalizing the
R3.1 preview/apply/undo surface into a review step for non-visual Designer
proposals, still inside the Editor and still behind the revision-guarded apply
choke point. R3.1 stays the reference slice: one action, one understandable
result, in product language.

## 16. R4.1 visual design foundation

R4.1 adds the shared graphics language the R-series needs before it can grow
beyond one image role. It separates three axes that people casually merge -
`VisualAssetRole` (where/how), `VisualIntent` (purpose) and `VisualPlacement`
(layout) - and resolves them from an Editor instruction plus context through a
conservative, deterministic, pure resolver: explicit role wins, explicit
intent/placement overrides the role default, editor node type only breaks ties,
ambiguity becomes a clarification question, and a non-visual request is refused
rather than guessed. The shared visual ranker now factors role/placement/aspect
...
the editor cannot host yet (hero, background and logo at R4.1; hero became
hostable in R4.3, background in R4.4, logo still stands) are reported as
...
are in `docs/visual-design-foundation.md`.

## 17. Handoff to the next briefing

R4.1 is done and R3.1 still works. The next step is to widen the *host* side so
more resolved roles become real, keeping every R4.1 invariant (three separate
axes, honest refusal over guessing, pure ranking, no new persistence, no new
route or detached UI). In priority order: host blocks for `hero` and full-bleed
`background`; replacement of an existing image block; candidate choice ("show me
another") reusing the same ranker and intent; then `logo`/`icon` hosts. The
general R4 review-before-apply step for non-visual Designer proposals remains
open. Details and rationale are in `docs/visual-design-foundation.md`.

## 18. R4.2 role-aware section image capability

R4.2 turns the R4.1 foundation into the first user-visible role-aware
capability: a **section** image, requested in the Editor as "Geef deze sectie
een passende afbeelding." (or English). A section is the document's real
structure, not a new abstraction: an explicit composition `section` container,
or a heading-delimited region (a heading plus its following blocks until the
next heading of the same or higher level). The Editor always sends its real
caret/selection as `target` and adds the addressed section as a structural
`sectionTarget` hint; the backend validates that hint against the canonical
snapshot with the same pure `resolveSectionVisual` the capability is defined
by, then ranks existing media against the section heading plus its bounded body
and inserts after the heading, before the first content block. The inline
candidate is unchanged from R3.1 (preview, role label, Insert / Cancel) and the
insertion is still one undoable editor transaction through the existing
`insertMedia` command; the backend still never writes the document. One role is
fully integrated on purpose; the other insertable roles keep their R4.1
...
## 20. R4.4 role-aware background visual capability

R4.4 turns the R4.2/R4.3 host pattern onto the third role: a **background**
image, requested in the Editor as "Gebruik een rustige achtergrond." (or
English). A background is **hosted** in the document's real structure, not a new
abstraction: an explicit section or hero container, or a heading-delimited
section / page-hero region. It keeps three dimensions separate: the **visual
role** (`background`), the **host region** (`section` or `hero`, reusing the
R4.2/R4.3 targets) and the **layout placement** (fixed to `full_bleed`).
`section`/`hero` are host regions, never new `VisualPlacement` values. The Editor
always sends its real caret/selection as `target` and adds a structural
`backgroundTarget` hint; the backend validates that hint against the canonical
snapshot with the same pure resolvers the capability is defined by, then ranks
existing media against the host heading plus its bounded body/supporting copy and
inserts after the heading. The image stays a real image block with
`role: "background"` and **empty alt** (decorative); R4.4 never mutates CSS
`background-image`. Any placement other than `full_bleed` is refused rather than
downgraded, and a named hero/section becomes the background's host region (the
R4.3 `hero_background_unsupported` outcome is removed). The inline candidate is
unchanged from R3.1/R4.2/R4.3 and the insertion is still one undoable editor
transaction; the backend still never writes the document. Full contract, error
table and limitations are in `docs/background-visual-capability.md`.

## 21. Handoff to the next briefing

R4.4 is done, R4.1/R4.2/R4.3/R3.1 still work. "Gebruik een rustige achtergrond."
now inserts a real full-bleed background image inside the Editor, while the
section, hero and plain image instructions are unchanged. The next step is to
widen the *host* side further while keeping every R4 invariant (three separate
...
replacement of an existing image block (including a hero/section/background
image); candidate choice ("show me another") reusing the same ranker and intent;
then `logo`/`icon` hosts. The general R4 review-before-apply step for non-visual
Designer proposals remains open. Details and rationale are in
`docs/background-visual-capability.md`, `docs/hero-visual-capability.md`,
...
- `docs/background-visual-capability.md` — R4.4 role-aware background visual
  capability.
