...
| REST v1 | `/api/v1/projects/:projectId/content` (+ analysis) behind SHA-256-hashed, revocable API keys with `read`/`write` scopes; shares the SEO Core services. Two key kinds: project keys (bound to one project) and account/master keys (reach any project the owner is a member of, resolved per request and never stronger than that membership) |
| MCP | Stdio + streamable-HTTP (`/api/mcp`) servers sharing one tool registry; tools `content_list/get/analyze/update/generate/resolve_images`, `jobs_list`, `schedule_list/create/reschedule/cancel`, `publication_list/get`, `designer_capabilities/execute/get_run/apply`; publish/archive demand explicit `confirm`, delete is not exposed; no direct Postgres/provider access. Project keys bind one project; account/master keys resolve + authorize the target project per tool call via the owner's membership. Designer is proposal-first: `designer_execute` submits a durable run (intent or plan) and returns it, `designer_get_run` exposes the proposal, review and visual provenance, and only `designer_apply` writes an explicitly approved proposal through the existing revision-guarded apply |
